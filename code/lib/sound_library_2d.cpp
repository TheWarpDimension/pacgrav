...
{
	// Allocate the buffer
	unsigned int numSamples = 2000; //g_prefsManager->GetInt("SoundBufferSize", 2000);
...
:	m_callback(NULL),
	...
	m_fillsRequested(0)
{
	ReleaseAssert(!g_soundLibrary2d, "SoundLibrary2d already exists");

	m_freq = 44100;//g_prefsManager->GetInt("SoundMixFreq", 44100);
	m_samplesPerBuffer = 2000;//g_prefsManager->GetInt("SoundBufferSize", 2000);
...
void SoundLibrary2d::TopupBuffer()
{
	if (m_wavOutput)
	{
		...
		if (GetHighResTime() > nextOutputTime)
		{
			...
			int samplesPerSecond = 44100;//g_prefsManager->GetInt("SoundMixFreq", 44100);
...
void SoundLibrary2d::StartRecordToFile(char const *_filename)
{
	...
	ReleaseAssert(m_wavOutput != NULL, "Couldn't create wave outout file %s", _filename);
}
