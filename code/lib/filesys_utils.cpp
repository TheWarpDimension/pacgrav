...
void FileDetails::SetFullPath(char const *fullPath)
{
	...
	ReleaseAssert(lastSlashIndex != -1, "FileDetails constructed without full path '%s'", m_fullPath.c_str());
	lastSlashIndex++;
...
void FileDetails::FormatTime(unsigned _time, char *buf)
{
	unsigned clockNow = time(NULL);
	struct tm *thenStruct = localtime((time_t*)&clockNow);
	struct tm nowStruct;
	memcpy(&nowStruct, thenStruct, sizeof(struct tm));
	thenStruct = localtime((time_t*)&_time);
...
bool CreateDirectory(char const *_directory)
{
	...
	std::string reducedDir = _directory;
...
static void CorrectPathCase(std::string &path)
{
