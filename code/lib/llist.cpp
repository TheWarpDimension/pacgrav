...
void LList<T>::PutDataAtIndex(const T &newdata, int index)
{
    ...
    else 
	{
        ...
        LListItem <T> *li = new LListItem <T> ();
        li->m_data = newdata;		
        li->m_previous = current->m_previous;
        li->m_next = current;
        if (current->m_previous) current->m_previous->m_next = li;
        current->m_previous = li;		
        ++m_numItems;
...
LListItem<T> *LList<T>::GetItem(int index) const
{
	...
	{
        DebugAssert(m_cached->m_next);
		m_cached = m_cached->m_next;
		m_cachedIndex++;
	}
	while (index < m_cachedIndex) 
	{
        DebugAssert(m_cached->m_previous);
		m_cached = m_cached->m_previous;
		m_cachedIndex--;
	}
...
void LList<T>::RemoveData(int index)
{
	...
	{
		// Currently cached item is valid, but it's index has changed
		m_cachedIndex--;
	}
