...
#define MOUSE_X	(g_inputManager->m_mouseX)
#define MOUSE_Y (g_inputManager->m_mouseY)
...
public:
	...
	bool		m_windowClosed;			// True if the user has just tried to close the window (either with the X in the top right, or "close" from the window's context menu)
...
public:
	...
	bool	UntypeKey		(char key);	// Removes the specified key from the m_keysTypes array. Returns false if key was not present

	void	RegisterMouseUpdateCallback(MouseUpdateCallback *func) { m_mouseCallback = func; }
};


extern InputManager *g_inputManager;
extern signed char g_keys[KEY_MAX];		// Is the key currently pressed
extern signed char g_keyDowns[KEY_MAX];	// Was the key pressed this frame (includes key repeats)
extern signed char g_keyUps[KEY_MAX];	// Was the key released this frame
